...
- 以上默认值面向严格的企业网关：2026-09-19 用只接受流式、拒绝 OpenAI 专有字段的模拟网关验证，Codex 在去掉 `parallel_tool_calls` 并降级 `json_schema` 后才全部通过；同日 Windows x64 真实模型验收中 Kimi 发送的 `reasoning_effort` 被严格网关拒绝，因此也默认去掉。引擎在同一回答中收到多个工具调用时仍能正确处理。
