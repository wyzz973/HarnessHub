...
控制台任务工作台默认使用“直接执行”：消息直接提交为所选引擎 Session 的 Run，多轮消息留在同一 Session，不经过规划。需要先审核计划时，在输入框下方把“执行模式”切换为“自动规划”，它调用下文的 Workflow 接口。新建任务会恢复为“直接执行”；从 `?workflow=` 链接或历史中打开计划时显示为“自动规划”。

自动规划要求已登记的真实 ACP 引擎，fake 不参与。规划阶段禁止工具调用；Full Access 下引擎的写文件和命令请求不经确认就会执行，规划 Run 更容易出现工具事件而被拒绝（见下文“自动拆任务”），所以比赛环境一般使用直接执行。控制台在 Full Access 下选择自动规划时会给出提示。
