...
本功能是工具文件管理及配置映射。引擎的登录、模型服务可用性和是否支持 MCP 仍由既有 Driver 能力契约决定。启用 MCP 的包可以绑定支持该能力的 ACP 引擎；当前 CLI Driver 会明确拒绝启用 MCP。Pi 与 OpenClaw 当前适配器不消费 ACP 的 mcpServers，绑定这些引擎不表示原生 MCP 可用，须以统一配置校验结果为准。纯 Skill 包可用于现有配置支持的引擎。包中代码的实际网络权限由运行环境决定，本模块不提供网络沙箱，也不会将任意 MCP 代码解释成安全代码。发行中的内置文件工具可只提供本地读取功能。
