...
| 文件 | 写入者 | 内容 |
|---|---|---|
| `<dataDir>/logs/gateway.log` | Gateway 进程 | `gateway.start/listen/stop/start_failed`、`model.configured`（统一模型 ID、地址与兼容设置，无密钥）；`http` 访问行；`session.create`（含 `engineLog` 路径）/`status`/`backend`；`run.accept/status/finish`（结束行含状态、公开错误、`ms` 与排队 `queuedMs`）；`permission.request/decide/applied`；`worker.spawn/ready/exit/fail`（pid、退出码与信号、是否预期）；每个 `model.call` 摘要以及 `engine.error`、`engine.installation`、`runtime.cleanup` |
