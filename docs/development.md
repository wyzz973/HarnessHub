...
初期本地 hooks 保持快速，CI 负责完整检查。不得以 `--no-verify`、全局关闭规则或删除检查绕开失败；发现检查误报时修正判定并加入反例。当前 [CI 配置](../.github/workflows/ci.yml)已建立但未在远端运行，本地 hooks 尚未安装；检查接入情况见 [检查入口](testing.md#检查入口与接入顺序)。
