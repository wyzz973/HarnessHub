...
- Native modes: Codex starts in `agent-full-access`, Gemini and Qwen use `--approval-mode yolo`, MiMo uses `--yolo`, Hermes uses `HERMES_YOLO_MODE=1`. OpenCode, DSH, Pi and OpenClaw rely on the ACP approval (OpenClaw's private exec policy is widened by its bundled launcher); OpenCode no longer receives `OPENCODE_PERMISSION`, which made fixed OpenCode runs fail.
