...
export async function installLocal(
  ...
  return mutation(requestedRoot, (root, reader) =>
    publishStaged(root, reader, (stage) =>
      inspectDirectory(sourceRoot, reader, async (file, bytes) =>
        writeExclusive(
...
/**
 * Installs a manifest generated by the caller (for example an import of a
 * Skill directory or mcp.json) whose payload bytes come from `read`. Every
 * returned buffer must match the declared size and SHA-256, otherwise the
 * source changed after the manifest was generated and nothing is registered.
 * Uses the same private staging, full re-verification, idempotence and
 * TOOL_PACKAGE_VERSION_CONFLICT rules as installLocal.
 */
export async function installGenerated(
  manifest: unknown,
  read: (file: ToolPackageFile) => Promise<Buffer>,
  requestedRoot: string,
): Promise<InstalledToolPackage> {
  const inspection = parseManifest(manifest);
  return mutation(requestedRoot, (root, reader) =>
    publishStaged(root, reader, async (stage) => {
      for (const file of inspection.manifest.files) {
        const bytes = await read(file);
        if (bytes.length !== file.size || hash(bytes) !== file.sha256)
          throw packageError(
            "TOOL_PACKAGE_CHANGED",
            `Source file ${file.path} changed while it was being imported`,
          );
        await writeExclusive(
          path.join(stage, ...file.path.split("/")),
          bytes,
...
/** Fills a private stage, re-verifies it, then publishes the object and record. */
async function publishStaged(
  root: string,
  reader: PackageReader,
  fill: (stage: string) => Promise<ToolPackageInspection>,
): Promise<InstalledToolPackage> {
  const stage = await mkdtemp(path.join(root, ".staging", "install-"));
  const stageIdentity = await lstat(stage, { bigint: true });
  let published = false;
  try {
    const inspection = await fill(stage);
    await writeExclusive(
    ...
    if (exists) {
      if ((await inspectDirectory(object, reader)).digest !== inspection.digest)
        throw packageError(
          "TOOL_PACKAGE_INTEGRITY",
...
/**
 * Reads the canonical manifest stored in a registered content object and
 * checks it against the record's digest and identity. Payload files are not
 * re-hashed, so this suits listings and binding-ownership lookups but is never
 * evidence that a package is intact (use verifyInstalled).
 */
export async function readManifest(
  ...
  try {
    const bytes = await reader.read(
      path.join(root, "objects", record.digest, MANIFEST_NAME),
    ...
    try {
      ...
      throw packageError(
        "TOOL_PACKAGE_INTEGRITY",
        "Installed manifest is not valid UTF-8 JSON",
      );
    }
    const inspection = parseManifest(value);
    if (
      inspection.digest !== record.digest ||
      inspection.manifest.id !== record.id ||
      inspection.manifest.version !== record.version
    )
      throw packageError(
        "TOOL_PACKAGE_INTEGRITY",
...
/** Records (optionally including removed ones) of one or all packages with their manifests; see readManifest. */
export async function listManifests(
  requestedRoot: string,
  options: { includeRemoved?: boolean; id?: string } = {},
): Promise<{ record: ToolPackageRecord; manifest: ToolPackageManifest }[]> {
  const records = (
    await listInstalled(requestedRoot, {
      includeRemoved: options.includeRemoved === true,
    })
  ).filter((record) => options.id === undefined || record.id === options.id);
  const result: { record: ToolPackageRecord; manifest: ToolPackageManifest }[] =
    [];
  for (const record of records)
    result.push({
      record,
      manifest: await readManifest(requestedRoot, record),
    });
  return result;
}
